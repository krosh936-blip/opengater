...
const UPSTREAM_BASE_URL = 'https://api.bot.eutochkin.com/api';
...
const proxyRequest = async (req: NextRequest, pathParts: string[]) => {
  try {
    ...
    // Эмулируем browser-like Origin/Referer для upstream.
    headers.set('origin', 'https://api.bot.eutochkin.com');
    headers.set('referer', 'https://api.bot.eutochkin.com/');
    headers.set(
      'user-agent',
